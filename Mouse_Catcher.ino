...
int irPin1 = 13;  // กำหนดพินที่ใช้เชื่อมต่อกับ IR Sensor ตัวแรก
int irPin2 = 10;  // กำหนดพินที่ใช้เชื่อมต่อกับ IR Sensor ตัวที่สอง
int irValue1;     // ตัวแปรเก็บค่าจาก IR Sensor ตัวแรก
int irValue2;     // ตัวแปรเก็บค่าจาก IR Sensor ตัวที่สอง

void setup() {
  Serial.begin(9600);
  myServo.attach(6);  // เชื่อมต่อ Servo เข้ากับพิน 9
...
void loop() {
  ...
  if (irValue1 == HIGH && irValue2 == LOW) {  
    ...
    delay(1000);  // รอ 0.5 วินาที
  }

  // เมื่อ IR Sensor 2 จับการเคลื่อนไหว (หนูออกจากกรง) และ IR Sensor 1 ไม่จับการเคลื่อนไหว
